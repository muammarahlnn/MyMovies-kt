...
import com.ardnn.mymovies.databinding.FragmentOnTheAirBinding
import com.ardnn.mymovies.listeners.SingleClickListener
import com.ardnn.mymovies.models.TvShowOutline

class OnTheAirFragment : Fragment(), SwipeRefreshLayout.OnRefreshListener,
    SingleClickListener<TvShowOutline> {

    // view binding
    private var _binding: FragmentOnTheAirBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentOnTheAirBinding.inflate(inflater, container, false)

        // set swipe refresh layout
        binding.srlOnTheAir.setOnRefreshListener(this)
    ...
    private fun loadData(page: Int) {
        TvShowRepository.getOnTheAirTvShows(page, object : TvShowOutlineCallback {
            override fun onSuccess(tvShowOutlineList: MutableList<TvShowOutline>) {
                ...
                // done fetching
                binding.pbOnTheAir.visibility = View.GONE
                isFetching = false
                binding.srlOnTheAir.isRefreshing = false
            }
    ...
    private fun setRecyclerView() {
        ...
        binding.rvOnTheAir.layoutManager = layoutManager

        // set adapter
        adapter = TvShowsPrimaryAdapter(tvShowList, this)
        binding.rvOnTheAir.adapter = adapter

        // listener if recyclerview reached last item then fetch next page
        binding.rvOnTheAir.addOnScrollListener(object : RecyclerView.OnScrollListener() {
    ...
    override fun onRefresh() {
        // reset fetching
        currentPage = 1
        binding.pbOnTheAir.visibility = View.VISIBLE
        loadData(currentPage)
    }
